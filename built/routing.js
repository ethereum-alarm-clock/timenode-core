...
var isClaimedByUs = function (conf, txRequest) {
    var ourClaim = conf.wallet
        ? conf.wallet.isKnownAddress(txRequest.claimedBy)
        : txRequest.isClaimedBy(conf.web3.eth.defaultAccount);
    if (!ourClaim)
        conf.logger.debug("[" + txRequest.address + "] In reserve window and not claimed by our account.");
    return ourClaim;
};
var getSender = function (conf) {
    return conf.wallet ? conf.wallet.getAddresses()[0] : conf.web3.eth.defaultAccount;
};
